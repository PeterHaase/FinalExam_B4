...
public class Runner {
	...
	public static void main(String[] args) {
		...
		JFrame frame = new JFrame("Whut Decimal?");
		
		int number = Integer.parseInt( JOptionPane.showInputDialog(frame, "Your Number"));
		
		if(number <= 255 && intBase10 >= 0){
			intBase10 = number;
		}else{
			JOptionPane.showMessageDialog(null, "That number is not within the bounds");
			System.exit(0);
		}
		
		hexAnswer = Integer.toHexString(number);
		// for testing, remove all syso when done
		//System.out.println("0-255 entry: " + intBase10 + ", hex conversion: " + hexAnswer);
		JOptionPane.showMessageDialog(null, "Your hex is : " + hexAnswer);
